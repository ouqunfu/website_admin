...
var DayTableMixin = FC.DayTableMixin = {
	...
	dayDates: null, // whole-day dates for each columns. left to right
	...
	// Computes a default columns header formatting string if `colFormat` is not explicitly defined
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	// isRigid determins whether the individual rows should ignore the columns and be a constant height.
	...
	/* Cell System
	------------------------------------------------------------------------------------------------------------------*/
	// FYI: the first columns is the leftmost columns, regardless of date
...
DayGrid.mixin({
	...
	renderSegRow: function(row, rowSegs) {
		...
		var loneCellMatrix = []; // lookup for <td> elements that only take up a single columns
		var i, levelSegs;
		var col;
		var tr;
		var j, seg;
		...
		// populates empty cells from the current columns (`col`) to `endCol`
...
DayGrid.mixin({
	...
	limitRow: function(row, levelLimit) {
		...
		var cellMatrix; // a matrix (by level, then columns) of all <td> jQuery elements in the row
		var limitedNodes; // array of temporarily hidden level <tr> and segment <td> DOM nodes
		var i, seg;
		var segsBelow; // array of segment objects below `seg` in the current `col`
		var totalSegsBelow; // total number of segments below `seg` in any of the columns `seg` occupies
		var colSegsBelow; // array of segment arrays, below seg, one for each columns (offset from segs's first columns)
		var td, rowspan;
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					// make a replacement <td> for each columns the segment occupies. will be one for each colspan
	...
	// Builds the inner DOM columns of the segment popover
...
TimeGrid.mixin({
	...
	colContainerEls: null, // containers for each columns

	// inner-containers for each columns where different types of segs live
	fgContainerEls: null,
	bgContainerEls: null,
	...
	// Given segments grouped by columns, insert the segments' elements into a parallel array of container
	// elements, each living within a columns.
	attachSegsByCol: function(segsByCol, containerEls) {
		var col;
		var segs;
		var i;

		for (col = 0; col < this.colCnt; col++) { // iterate each columns grouping
			segs = segsByCol[col];
	...
	// and attaches to the columns inner-container elements.
	...
	// Given segments that are assumed to all live in the *same columns*,
	...
	// Given an array of segments that are all in the same columns, sets the backwardCoord and forwardCoord on each.
...
var BasicView = FC.BasicView = View.extend({
	...
	// Generates an HTML attribute string for setting the width of the week number columns, if it is known
...
var agendaTimeGridMethods = {
	...
	// Generates the HTML that goes before the bg of the TimeGrid slot area. Long vertical columns.
	renderBgIntroHtml: function() {
		var view = this.view;
